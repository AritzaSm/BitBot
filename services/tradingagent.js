...
agent.prototype.calculateOrder = function(result) {
  ...
  if(this.orderDetails.orderType === 'buy') {
    ...
    var lowestAskWithSlippage = tools.round(lowestAsk * (1 + (this.slippagePercentage / 100)), 5);
    ...
    this.orderDetails.amount = tools.floor(balance / this.orderDetails.price, 5);

    this.simulationBalance = {assetAvailable: tools.round(this.orderDetails.assetAvailable +  this.orderDetails.amount,5), currencyAvailable: 0, fee: this.orderDetails.transactionFee};

  } else if(this.orderDetails.orderType === 'sell') {

    var highestBid = lastClose;

    var highestBidWithSlippage = tools.round(highestBid * (1 - (this.slippagePercentage / 100)), 5);
    ...
    this.orderDetails.amount = tools.round(this.orderDetails.assetAvailable - this.tradingReserveAsset, 5);

    this.simulationBalance = {assetAvailable: 0, currencyAvailable: tools.round(this.orderDetails.currencyAvailable + (this.orderDetails.amount * this.orderDetails.price), 5), fee: this.orderDetails.transactionFee};
...
agent.prototype.placeRealOrder = function() {
  ...
  if(this.orderDetails.amount <= 0.1) {
